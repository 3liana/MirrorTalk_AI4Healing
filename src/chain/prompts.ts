...
export function getTurnGeneratorPrompt(
  ...
  const goodThingsQuestionBank = role.id === "haodongxi" ? `

好东西场景题库（仅在角色 id=haodongxi 时使用；每轮可随机抽取 1 题，避免重复；保留原有 hashtags 和场景回顾，用“你的觉察：”作为提问）：

Q1 场景：为了男人吃安眠药。热议点：#小叶恋爱脑# #为了爱值得吗# 剧情回顾：小叶为了让胡医生觉得自己“特别”，故意扮演一个破碎的、离不开他的女孩。你的觉察：这种“表演”，对你来说意味着什么？
- A. 这是我的狩猎策略（维度：抵抗方式-妥协利用）
- B. 这让我感到被需要（维度：价值确认路径-关系维持）
- C. 我在掌控这个剧本（维度：欲望可见度-隐形掌控）
- D. 我分不清哪个是真的（维度：自我感稳定性-模糊）

Q2 场景：不想做主角。热议点：#我想做观众# #正是因为我们都在这里# 剧情回顾：茉莉说“不想做主角，想做观众”。你的觉察：当你听到这句话时，内心深处涌上来的是什么感觉？
- A. 巨大的释放（维度：自由位置-自我许可）
- B. 隐隐的担忧（维度：责任取向-过度担当）
- C. 觉醒的力量（维度：位置感-审视者掌控）

Q3 场景：深夜看差评。热议点：#为什么要把恶评挂在墙上# 剧情回顾：铁梅表面不在乎恶评，但深夜会逐条翻看、甚至反驳。你的觉察：当你做得很好却依然听到刺耳的声音时，内心的真实反应是？
- A. 战斗本能（维度：抵抗方式-正面对抗）
- B. 隐秘的认同（维度：价值确认路径-依赖外部）
- C. 傲慢的隔离（维度：抵抗方式-内在抽离）

Q4 场景：饭桌修罗场。热议点：#饭桌上的雄竞# #男人太爱说教了# 剧情回顾：饭桌上两位男性大谈女性主义、争高低，无视在场女性。你的觉察：坐在那张桌子上，本能反应是？
- A. 愤怒的反击者（维度：抵抗方式-直接对抗）
- B. 戏谑的观察者（维度：抵抗方式-柔性解构）
- C. 疲惫的离席者（维度：抵抗方式-回避/节能）

使用规则（内部提醒）：
- 仅在“好东西”世界启用，不暴露“维度”字样给用户
- 每轮最多抽取 1 题，优先未使用题；如轮次不足按 Q1→Q2→Q3→Q4 顺序
- 维度信息仅用于开发侧理解，不输出给用户
` : "";
...
${module === "action" && turnIndex >= Math.max(maxTurns - 2, 8) ? `⚠️ 这是倒数轮，如果感觉对话已足够深，可以将 endCondition.canEnd 设为 true` : ""}`;
  return basePrompt + goodThingsQuestionBank;
}
